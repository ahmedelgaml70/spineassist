...
The camera is fitted in its projected teaching plane across the sampled motion arc, so intermediate
humeral positions cannot escape an endpoint-only fit. GH seating is independently audited.
"""
...
def projected_camera(name, meshes, direction, scene, frames=(1,10,20,30,40,50,60), margin=1.18):
    all_pts=[]; per_frame={}
    for f in frames:
        scene.frame_set(f); base.bpy.context.view_layer.update(); pts=[]
        for o in meshes: pts.extend(base.verts(o,5000))
        per_frame[f]=pts; all_pts.extend(pts)
    if not all_pts: raise RuntimeError('No geometry for projected camera fit')
    centroid=sum(all_pts,Vector((0,0,0)))/len(all_pts); span3=max((p-centroid).length for p in all_pts)*2
    ...
    scale=max(height,width/aspect)*margin; cx=(minx+maxx)/2; cy=(miny+maxy)/2
    right=cam.matrix_world.to_quaternion()@Vector((1,0,0)); up=cam.matrix_world.to_quaternion()@Vector((0,1,0)); cam.location += right*cx + up*cy; cam.data.ortho_scale=scale; scene.camera=cam
    base.bpy.context.view_layer.update(); inv=cam.matrix_world.inverted(); half_h=scale/2; half_w=half_h*aspect; min_clear=1.0
    edge=[]
    for f,pts in per_frame.items():
        c=[inv@p for p in pts]; clearance=min(half_w-max(abs(p.x) for p in c),half_h-max(abs(p.y) for p in c)); ratio=clearance/min(half_w,half_h); edge.append({'frame':f,'clearance_ratio':ratio}); min_clear=min(min_clear,ratio)
    fill=max(height/scale,width/(scale*aspect)); qa['camera_fit'].append({'clip':name,'sampled_frames':list(frames),'projected_width':width,'projected_height':height,'ortho_scale':scale,'frame_fill_ratio':fill,'margin':margin,'minimum_edge_clearance_ratio':min_clear,'edge_clearance':edge})
    if fill<0.68 or fill>0.88: raise RuntimeError(f'{name} projected teaching-frame fill {fill:.3f} outside 0.68-0.88')
    if min_clear<0.045: raise RuntimeError(f'{name} minimum sampled edge clearance {min_clear:.3f} below 0.045')
    return cam
def clean_render(name,h,s,c,j_unused,mode):
    ...
    direction=Vector((1,0,0.03)).normalized() if mode=='flexion' else Vector((0,-1,0.03)).normalized(); cam=projected_camera(name,[q,s,c],direction,sc)
...
p=base.OUT/'model_manifest.json'; m=json.loads(p.read_text()); max_dup=max((x['shift'] for x in qa['duplicates']),default=0); max_par=max((x['shift'] for x in qa['parenting']),default=0); max_rad=max((x['error'] for x in qa['pivot_radius']),default=0); max_ratio=max((x['drift']/x['limit'] for x in qa['gh_contact']),default=0); max_head=max((x['translation'] for x in qa['head_center']),default=0); min_edge=min((x['minimum_edge_clearance_ratio'] for x in qa['camera_fit']),default=0)
passed=max_dup<=1e-5 and max_par<=1e-5 and max_rad<=1e-4 and max_ratio<=1 and max_head<=1e-5 and min_edge>=.045 and len(qa['gh_contact'])==2 and len(qa['camera_fit'])==2
m['pipeline_qa']={'fix_version':'full-arc-projected-camera-v12','hierarchy_contact_preserved':passed,'clean_renderer_executed':len(qa['gh_contact'])==2,'decorative_arc_created':False,'max_source_to_baked_center_shift':max_dup,'max_parenting_world_center_shift':max_par,'max_pivot_radius_error':max_rad,'max_gh_contact_drift_ratio':max_ratio,'max_humeral_head_center_translation':max_head,'minimum_sampled_edge_clearance_ratio':min_edge,'events':qa,'motion_geometry':'world-baked humerus rotated about locally estimated humeral-head centre','teaching_scope':'isolated glenohumeral motion; scapula and clavicle deliberately fixed','temporal_rule':'neutral to named terminal pose, then hold; no antagonist return','poster_rule':'terminal movement pose','camera_rule':'orthographic teaching view fitted from projected geometry sampled across the full motion arc with 16:10 aspect-aware safety margin and per-sample edge-clearance QA','visual_cue':'moving humerus only; no decorative 3D arc'}
m['selection_debug']['visible_rule']='only matched scapula, clavicle, and moving humerus; imported source humerus hidden'; m['selection_debug']['corrective_change']='v12 samples the motion arc at frames 1/10/20/30/40/50/60, fits their union in camera space, increases safety margin, and QA-gates minimum edge clearance at every sampled pose.'
