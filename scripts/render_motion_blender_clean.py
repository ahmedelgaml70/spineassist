...
Renders genuine motion from the matched meshes in the complete body.glb.  The clip is deliberately
start -> end rather than start -> end -> start: a teaching slide should show one named anatomical
action unambiguously, not a motion followed by its antagonist under the same label.
"""
...
def clean_render(name,h,s,c,j,mode):
    ...
    # One direction only. Holding the terminal position makes the named action legible and prevents
    # the second half of a "flexion" clip from actually depicting extension (and vice versa).
    ...
    # Orthogonal views make the intended plane easier to read: lateral-ish for flexion, anterior-ish for abduction.
    ...
    sc=base.bpy.context.scene; sc.frame_set(1); r0=sorted((p-j).length for p in base.verts(q,500)); _,_,d0=base.closest(q,s,500)
    sc.frame_set(60); r1=sorted((p-j).length for p in base.verts(q,500)); _,_,d1=base.closest(q,s,500)
    radial=max((abs(a-b) for a,b in zip(r0,r1)),default=0.0); drift=abs(d1-d0); limit=max(.004,base.extent(h)*.04)
    qa['pivot_radius'].append({'clip':name,'error':radial}); qa['gh_contact'].append({'clip':name,'start':d0,'end':d1,'drift':drift,'limit':limit})
    print(f'CLEAN_RENDER_ACTIVE {name} radial={radial:.9f} contact_start={d0:.6f} contact_end={d1:.6f} drift={drift:.6f} limit={limit:.6f}')
    if radial>1e-4 or drift>limit: raise RuntimeError(f'GH QA failed for {name}')
    # Poster uses the terminal pose, so a static slide visibly communicates the named movement.
...
p=base.OUT/'model_manifest.json'; m=json.loads(p.read_text()); max_dup=max((x['shift'] for x in qa['duplicates']),default=0); max_par=max((x['shift'] for x in qa['parenting']),default=0); max_rad=max((x['error'] for x in qa['pivot_radius']),default=0); max_ratio=max((x['drift']/x['limit'] for x in qa['gh_contact']),default=0)
passed=max_dup<=1e-5 and max_par<=1e-5 and max_rad<=1e-4 and max_ratio<=1 and len(qa['gh_contact'])==2
m['pipeline_qa']={'fix_version':'pedagogic-unidirectional-v8','hierarchy_contact_preserved':passed,'clean_renderer_executed':len(qa['gh_contact'])==2,'decorative_arc_created':False,'max_source_to_baked_center_shift':max_dup,'max_parenting_world_center_shift':max_par,'max_pivot_radius_error':max_rad,'max_gh_contact_drift_ratio':max_ratio,'events':qa,'motion_geometry':'world-baked humerus about estimated GH pivot','teaching_scope':'isolated glenohumeral motion; scapula and clavicle deliberately fixed','temporal_rule':'neutral to named terminal pose, then hold; no antagonist return under the same label','poster_rule':'terminal movement pose','visual_cue':'moving humerus only; no decorative 3D arc'}
m['selection_debug']['visible_rule']='only matched scapula, clavicle, and moving humerus; imported source humerus hidden'; m['selection_debug']['corrective_change']='v8 changes each teaching clip from out-and-back to a single named action with terminal hold; posters now show the terminal pose.'
